<template>
	<view class="begin_test_content minHeight100">
		...
		<view class="paddingAll color9 fontSize16">
			{{beginTestContent.mobileDescription}}
		</view>
	...
	export default {
		...
		onShareAppMessage(options) {
			...
			var shareObj = {
				...
				path: '/scalePage/beginTestCognitiveTask/index?params=' + params, // 默认是当前页面，必须是以‘/'开头的完整路径
		...
		methods: {
			navToBeginTestAnswer() { 
				if (_this.$store.state.userInfo.user.id == "") {
					 console.log("==获取id===2===>",_this.$store.state.userInfo.user.id)
					getApp().jumpToTestRecordLogin()
				} else {
					console.log("==获取id===3===>",_this.beginTestContent)
					uni.setStorage({
						key: 'subjectInfo',
						data: JSON.stringify(this.beginTestContent),
						success: () => {
							uni.navigateTo({
								url: `/cognitiveTasksPage/${this.beginTestContent.pageUrl}`
								// url: getApp().getUrl(`/cognitiveTasksPage/${_this.beginTestContent.pageUrl}?item=`
								//  + encodeURIComponent(JSON.stringify(
								// 	_this.beginTestContent.mobileTestDescription)))
